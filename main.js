...
class Main {
  constructor() {
    ...
    // Add this to the constructor after other button declarations
    this.liveTranslateButton = document.getElementById("liveTranslateButton");
    this.isLiveTranslating = false;
  }
  ...
  createTranslateBtn() {
    ...
    this.liveTranslateButton.style.display = "block";
    this.createBackToTrainBtn();

    // Initialize live translation here
    this.initializeLiveTranslation();
    ...
    this.predButton.addEventListener('click', () => {
        ...
        if (Math.max(...exampleCount) > 0) {
            ...
            alert('You need to add training examples first.\nTrain each gesture with about 30 samples.');
        }
    });
}
...
setupTranslationUI() {
    ...
    this.stageTitle.innerText = "Live Detection";
    this.stageInstruction.innerText = "Start signing to see live translation";

    // Remove training UI
    this.trainingContainer.style.display = "none";
    this.trainedCardsHolder.style.marginTop = "130px";

    // Display translation holder
  ...
  initializeLiveTranslation() {
    this.liveTranslateButton.addEventListener('click', () => {
        const exampleCount = this.knn.getClassExampleCount();
        if (Math.max(...exampleCount) > 0) {
            if (!this.isLiveTranslating) {
                // Start live translation
                this.isLiveTranslating = true;
                this.liveTranslateButton.textContent = "Stop Live Translation";
                this.liveTranslateButton.classList.add("active");
                this.setupTranslationUI();
                this.setStatusText("Status: Live Translation Active", "predict");
                this.startLiveTranslation();
            } else {
                // Stop live translation
                this.isLiveTranslating = false;
                this.liveTranslateButton.textContent = "Start Live Translation";
                this.liveTranslateButton.classList.remove("active");
                this.setStatusText("Status: Live Translation Stopped", "predict");
                this.pausePredicting();
            }
        } else {
            alert('You need to add training examples first.\nTrain each gesture with about 30 samples.');
        }
...
predictLive() {
    if (!this.isLiveTranslating) return;
            ...
            if (Math.max(...exampleCount) > 0) {
                this.knn.predictClass(image)
                    .then((res) => {
                        for (let i = 0; i < words.length; i++) {
                            if (res.classIndex == i && 
                                ...
                                this.setStatusText("Status: Sign Detected!", "predict");
                                
                                // Update UI and speak the detected word
                                this.predictionOutput.textOutput(words[i], this.gestureCards[i], res.confidences[i] * 100);
                                this.previousPrediction = res.classIndex;
                            }
                        }
                    ...
                    .catch(error => {
                        ...
                        this.setStatusText("Status: Detection Error", "predict");
                    });
            }
        }
    }
    ...
    requestAnimationFrame(() => this.predictLive());
}
}
...
class PredictionOutput {
  ...
  textOutput(word, gestureCard, gestureAccuracy) {
    ...
    if (word == "start") {
      ...
      // Always speak detected words
      this.speak(word);
    }

    // Update gesture card display
